...
class GGConfig(object):
    ...
    """
    config_fp = os.environ.get('GG_CONFIG_FP', DEFAULT_CONFIG_FP)
    ...
    def __init__(self, config_fp=None):
        ...
        _expected_sections = {'main', 'postgres', 'tornado', 'ipython'}
    ...
    def _get_postgres(self, config):
        ...
        self.db_salt = config.get('postgres', 'salt')

    def _get_tornado(self, config):
        """Get tornado config bits"""
        self.http_port = config.getint('tornado', 'port')

    def _get_compute(self, config):
        """Get compute relevant bits"""
        self.compute_profile = config.get('ipython', 'profile')
